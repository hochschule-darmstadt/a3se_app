...
export default function StockItemDetailRoute() {
  ...
  const query = useApiQuery(["stock-items", stockItemId], () => apiClient.GET("/stock-items/{stock_item_id}", { params: { path: { stock_item_id: stockItemId as string } } }), { enabled: Boolean(stockItemId) });
  if (!stockItemId) return <StaffShell breadcrumbs={[{ label: "Inventory", to: "/stock-items" }, { label: "Detail" }]}><StatusBanner kind="error" title="No stock item specified" /></StaffShell>;
  if (query.status === "pending") return <StaffShell breadcrumbs={[{ label: "Inventory", to: "/stock-items" }, { label: "Detail" }]}><StatusBanner kind="loading" title="Loading stock item…" /></StaffShell>;
  if (query.status === "error") return <StaffShell breadcrumbs={[{ label: "Inventory", to: "/stock-items" }, { label: "Detail" }]}><ApiErrorBanner error={query.error} onRetry={() => query.refetch()} /></StaffShell>;
  return <StaffShell breadcrumbs={[{ label: "Inventory", to: "/stock-items" }, { label: query.data.productDisplayName }]}><StockDetail item={query.data} /></StaffShell>;
}
...
function StockDetail({ item }: { readonly item: StockItem }) {
  ...
  return <Stack gap="md"><Group justify="space-between"><div><Title order={1}>{item.productDisplayNameChain.join(" · ")}</Title><Text c="dimmed">{item.entityId} · {item.availabilityState}</Text></div><Group><Button variant="default" onClick={() => setEditing((value) => !value)}>{editing ? "Cancel editing" : "Edit stock entry"}</Button><Button color="red" variant="light" disabled={item.properties.inventoryStatusCode === "inventory/withdrawn"} loading={withdraw.isPending} onClick={() => withdraw.mutate(undefined, { onSuccess: refresh })}>Withdraw capacity</Button></Group></Group>
    <ResourceCard title={item.productDisplayName} subtitle={`${item.properties.serviceDate} · ${item.type}`} badge={item.availabilityState} details={[
      { label: "Product", value: item.productDisplayName },
      { label: "Supplier", value: item.supplierDisplayName ?? "Not linked" },
      { label: "Capacity", value: String(item.properties.capacityQuantity) }, { label: "Available", value: String(item.availableQuantity) }, { label: "Held", value: String(item.properties.heldQuantity) }, { label: "Allocated", value: String(item.properties.allocatedQuantity) },
      { label: "Unit sale price", value: `${item.properties.unitPriceAmount} ${item.properties.currencyCode}` },
    ]} />
    <Group><Anchor component={Link} to={`/products/${item.productId}`}>Open product</Anchor>{item.supplierOrganisationId ? <Anchor component={Link} to={`/organisations/${item.supplierOrganisationId}`}>Open supplier</Anchor> : null}</Group>
    {editing ? <form aria-label="Edit stock entry" onSubmit={save}><Stack gap="xs"><Title order={2}>Adjust capacity</Title><Group grow><TextInput type="date" label="Service date" value={date} onChange={(e) => setDate(e.currentTarget.value)} /><NumberInput min={0} label="Unit sale price (EUR)" value={price} onChange={setPrice} /><NumberInput min={0} label="Capacity" value={capacity} onChange={setCapacity} /><NumberInput min={0} label="Held" value={held} onChange={setHeld} /><Select label="Lifecycle" data={[{ value: "inventory/active", label: "Active" }, { value: "inventory/withdrawn", label: "Withdrawn" }, { value: "inventory/expired", label: "Expired" }]} value={lifecycle} onChange={(value) => setLifecycle((value as typeof lifecycle) ?? "inventory/active")} /></Group><Button type="submit" loading={update.isPending}>Save changes</Button>{update.isError ? <ApiErrorBanner error={update.error} /> : null}</Stack></form> : null}
    <Stack gap="xs"><Title order={2}>Order allocation</Title><Text size="sm" c="dimmed">Link or release this stock entry for one existing order position. Quantity is one per position in the current order model.</Text><Group grow><TextInput label="Order ID" value={orderId} onChange={(e) => setOrderId(e.currentTarget.value)} /><TextInput label="Order position ID" value={positionId} onChange={(e) => setPositionId(e.currentTarget.value)} /></Group><Group><Button disabled={!orderId.trim() || !positionId.trim() || item.availableQuantity <= 0 || item.availabilityState === "withdrawn"} loading={allocate.isPending} onClick={() => allocationAction("allocate")}>Allocate to order</Button><Button variant="default" disabled={!orderId.trim() || !positionId.trim()} loading={release.isPending} onClick={() => allocationAction("release")}>Release allocation</Button>{orderId.trim() ? <Button component={Link} variant="subtle" to={`/orders/${orderId.trim()}`}>Open order</Button> : null}</Group>{actionMessage ? <StatusBanner kind="success" title={actionMessage} /> : null}{allocationError ? <ApiErrorBanner error={allocationError} /> : null}</Stack>
