...
describe("StockItemsRoute", () => {
  ...
  it("opens the selected display-name chain in the right detail pane", async () => { const stock = item("STK-1", "Airport transfer", "available"); getMock.mockImplementation((path: string) => Promise.resolve({ data: path.includes("{stock_item_id}") ? stock : { items: [stock], nextCursor: null }, response: { ok: true, status: 200 } })); renderRoute(); const chain = await screen.findByText("Example Mobility · Mobility · Airport transfer"); await userEvent.click(chain); expect(await screen.findByRole("heading", { name: "Example Mobility · Mobility · Airport transfer" })).toBeInTheDocument(); expect(screen.getByRole("button", { name: /edit stock entry/i })).toBeInTheDocument(); });
