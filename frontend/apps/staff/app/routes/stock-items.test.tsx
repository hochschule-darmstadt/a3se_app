...
function item(id: string, name: string, state: "available" | "allocated") {
  return { entityId: id, entityKind: "StockItem", type: "stock/mobility/transfer", schemaVersion: 1, productId: `PRD-${id}`, productType: "product/mobility/transfer", productDisplayName: name, productDisplayNameChain: ["Example Mobility", "Mobility", name], productAncestors: [], supplierRole: null, supplierOrganisationId: "ORG-1", supplierDisplayName: "Example Mobility", availabilityState: state, properties: { serviceDate: "2027-05-12", unitPriceAmount: "50.00", currencyCode: "EUR", capacityQuantity: 5, remainingCapacity: state === "available" ? 4 : 2, inventoryStatusCode: "inventory/active" } };
}
...
describe("StockItemsRoute", () => {
  it("offers only product-aligned stock types with slash-separated labels", () => {
    expect(STOCK_TYPES).toContain("stock/accommodation/room-type");
    expect(STOCK_TYPES).toContain("stock/airline/flight");
    expect(STOCK_TYPES).toContain("stock/water-transport/day-boat");
    expect(STOCK_TYPE_OPTIONS.find(({ value }) => value === "stock/airline/flight")?.label).toBe("airline/flight");
  });
  it("renders availability", async () => { getMock.mockResolvedValue({ data: { items: [item("STK-1", "Airport transfer", "available")], nextCursor: null }, response: { ok: true, status: 200 } }); renderRoute(); expect(await screen.findByText(/airport transfer/i)).toBeInTheDocument(); const row = screen.getAllByRole("row")[1]!; expect(within(row).getByText("available")).toBeInTheDocument(); expect(within(row).getByText("4")).toBeInTheDocument(); expect(screen.getByRole("columnheader", { name: "Remaining" })).toBeInTheDocument(); expect(screen.queryByRole("columnheader", { name: "Available" })).not.toBeInTheDocument(); });
  it("sends search to the API instead of filtering the current page", async () => { getMock.mockImplementation((_path: string, options: { params: { query: { search?: string } } }) => Promise.resolve({ data: { items: options.params.query.search ? [item("STK-2", "Rail connection", "allocated")] : [item("STK-1", "Airport transfer", "available")], nextCursor: null }, response: { ok: true, status: 200 } })); renderRoute(); await screen.findByText(/airport transfer/i); await userEvent.type(screen.getByRole("textbox", { name: /^search$/i }), "rail"); await waitFor(() => expect(getMock).toHaveBeenLastCalledWith("/stock-items", expect.objectContaining({ params: { query: expect.objectContaining({ search: "rail" }) } }))); expect(await screen.findByText(/rail connection/i)).toBeInTheDocument(); });
