...
describe("StockItemsRoute", () => {
  ...
  it("opens the dated display-name chain in the right detail pane", async () => { const stock = item("STK-1", "Airport transfer", "available"); getMock.mockImplementation((path: string) => Promise.resolve({ data: path.includes("{stock_item_id}") ? stock : { items: [stock], nextCursor: null }, response: { ok: true, status: 200 } })); renderRoute(); const chain = await screen.findByText("2027-05-12 · Example Mobility · Mobility · Airport transfer"); await userEvent.click(chain); expect(await screen.findByRole("heading", { level: 2, name: "2027-05-12 · Example Mobility · Mobility · Airport transfer" })).toBeInTheDocument(); expect(screen.getByRole("button", { name: /edit stock entry/i })).toBeInTheDocument(); expect(screen.queryByRole("button", { name: /allocate to order/i })).not.toBeInTheDocument(); expect(screen.queryByRole("button", { name: /withdraw capacity/i })).not.toBeInTheDocument(); });
