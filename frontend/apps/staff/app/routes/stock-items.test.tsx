...
function item(id: string, name: string, state: "available" | "allocated") {
  return { entityId: id, entityKind: "StockItem", type: "stock/mobility/transfer", schemaVersion: 1, productId: `PRD-${id}`, productType: "product/mobility/transfer", productDisplayName: name, productDisplayNameChain: ["Example Mobility", "Mobility", name], productAncestors: [], supplierRole: null, supplierOrganisationId: "ORG-1", supplierDisplayName: "Example Mobility", availableQuantity: state === "available" ? 4 : 2, availabilityState: state, properties: { serviceDate: "2027-05-12", unitPriceAmount: "50.00", currencyCode: "EUR", capacityQuantity: 5, remainingCapacity: state === "available" ? 4 : 2, inventoryStatusCode: "inventory/active" } };
}
...
describe("StockItemsRoute", () => {
  ...
  it("renders availability", async () => { getMock.mockResolvedValue({ data: { items: [item("STK-1", "Airport transfer", "available")], nextCursor: null }, response: { ok: true, status: 200 } }); renderRoute(); expect(await screen.findByText(/airport transfer/i)).toBeInTheDocument(); const row = screen.getAllByRole("row")[1]!; expect(within(row).getByText("available")).toBeInTheDocument(); expect(within(row).getAllByText("4").length).toBeGreaterThan(0); });
