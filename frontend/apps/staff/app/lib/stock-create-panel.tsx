...
export function StockCreatePanel({ onCancel, onCreated }: { readonly onCancel: () => void; readonly onCreated: (id: string) => void }) {
  ...
  const mutation = useApiMutation<StockItem, void>(() => apiClient.POST("/stock-items", { body: { productId: productId.trim(), type, properties: { serviceDate, unitPriceAmount: String(price), currencyCode: "EUR", capacityQuantity: Number(capacity), remainingCapacity: Number(capacity), inventoryStatusCode: "inventory/active", searchText: `${productId.trim()} ${serviceDate}` } } }));
