...
export function OrderDetailPanel({ orderId, positionHref }: { readonly orderId: string; readonly positionHref?: (positionId: string) => string }) {
  ...
  const stockQueries = useQueries({ queries: positions.map(position => ({ queryKey: ["stock-items", position.stockItemId], enabled: Boolean(position.stockItemId), queryFn: async () => { const { data, error } = await apiClient.GET("/stock-items/{stock_item_id}", { params: { path: { stock_item_id: position.stockItemId! } } }); if (error) throw error; return data; } })) });
  ...
  return <Stack gap="md"><Group justify="space-between" align="flex-start"><Title order={1}>Order {detail.order.properties.orderNumber}</Title><Badge>{detail.order.properties.orderStatusCode.replace("order/", "")}</Badge></Group>
    ...
    <div><Title order={2}>Positions</Title><Stack gap="md" mt="xs">{positions.length === 0 ? <StatusBanner kind="empty" title="This order has no positions."/> : positions.map((position, index) => { const stock = stockQueries[index]?.data; return <Stack key={position.positionId} gap={6} p="sm" style={{ border: "1px solid var(--mantine-color-gray-3)", borderRadius: "var(--mantine-radius-sm)" }}><Group justify="space-between">{positionHref ? <Text component={Link} to={positionHref(position.positionId)} fw={600}>{position.positionId}</Text> : <Text fw={600}>{position.positionId}</Text>}{position.stockItemId ? null : <Badge color="orange">Unresolved</Badge>}</Group><PositionHierarchies orderId={orderId} orderNumber={detail.order.properties.orderNumber} customerRoleId={detail.customerRoleId} customerPersonId={detail.customerPersonId} customerDisplayName={detail.customerDisplayName} position={position} stock={stock}/></Stack>; })}</Stack></div>
  </Stack>;
}
