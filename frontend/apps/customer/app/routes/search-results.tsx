  ...
  return <CustomerShell><Container py="xl" size="lg"><Stack gap="lg">
    ...
    {query.isSuccess ? query.data.items.length === 0 ? <StatusBanner kind="empty" title={t("results.empty")} /> : <>
      {groups.map(([type, products]) => <Stack key={type} gap="sm"><Title order={2}>{type.replace(/^product\//, "").replaceAll("/", " · ")}</Title><SimpleGrid cols={{ base: 1, sm: 2, lg: 3 }} spacing="lg">
        {products.map((product) => <ResourceCard key={product.productId} title={product.productDisplayNameChain.join(" · ")} subtitle={product.productId} badge={type.replace(/^product\//, "")} details={[{ label: t("results.price"), value: `${product.indicativeUnitPriceAmount} ${product.currencyCode}` }]} action={<Stack gap="xs"><Select aria-label={`${product.productDisplayName} date`} placeholder={t("results.chooseDate")} data={product.availableDates} value={selectedDates[product.productId] ?? null} onChange={(value) => setSelectedDates((current) => ({ ...current, [product.productId]: value ?? "" }))} /><Link to={detailHref(product.productId)}>{t("results.viewDetail")}</Link></Stack>} />)}
      </SimpleGrid></Stack>)}
