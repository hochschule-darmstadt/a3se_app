...
| `remainingCapacity` | non-negative integer | Traveller capacity not yet allocated to order positions; the persisted source of truth for availability. Allocation subtracts, and release adds back, one unit per traveller assigned to the position. | Inventory; all current StockItem types | defaults to 1; must not exceed `capacityQuantity` | Project extension for INV-001; DR-0027 |
