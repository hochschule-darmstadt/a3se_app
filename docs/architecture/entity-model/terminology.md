...
| `roleStatusCode` | namespaced code | Current `PersonRole` lifecycle status; follows the `orderStatusCode` pattern (TERM-005). Deactivating a role (VIEW-S-002) sets `role/inactive`; it does not delete the `Person` or `PersonRole` record. | Person Management; `person/customer`, `person/traveller` | required; defaults to `role/active` when omitted; the CRUD API's `PUT` is the only current transition mechanism, so any code may currently follow any other -- ordering/guard rules remain future requirements work, matching `orderStatusCode`'s unresolved scope | Project value set; no external lifecycle adopted |
| `role/active` | namespaced code | The role is currently in effect. | Person Management; `roleStatusCode` | permitted; default | Project extension |
| `role/inactive` | namespaced code | The role has been deactivated and is excluded by default from the "active" role-status filter. | Person Management; `roleStatusCode` | permitted | Project extension; issue #29 |
