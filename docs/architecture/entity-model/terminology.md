...
- DR-0017 renamed `partner/supplier/*` to `organisation/*` and aligned/nested several `product/*` identifiers (see TERM-009), but deliberately left every `stock/*` identifier unrenamed, following the precedent already recorded for `stock/flight/seat` in this table's TERM-002 row. `stock/water/day-boat` and `stock/water/cruise` now read inconsistently against their renamed `product/water-transport/day-boat`/`product/water-transport/cruise` counterparts, and `stock/accommodation/room-category` against the renamed `product/accommodation/room-type`; renaming the `stock/*` side would touch Inventory (VIEW-S-007) and Order Management beyond this change's scope and remains a named follow-up.
...
Added for issue #12 to cover the portal images that seed data attaches to representative touristic resources. Reduced to a single property by DR-0018, which superseded this term's original eight-property, licence-attribution shape; see DR-0018 for rationale and the accepted licence-compliance tradeoff.
...
Stakeholder direction (2026-08-21): the stakeholder directed that TERM-010 be reduced to `imageUrl` only, removing `imageSourcePageUrl`, `imageCreatorCredit`, `imageLicenceCode`, `imageLicenceVersion`, `imageAttributionText`, `imageAltText`, and `imageVerifiedDate`, along with the all-or-nothing validator and the seed alt-text-not-derived-from-filename check. AI flagged before making the change that the removed fields exist specifically to satisfy the attribution terms of the CC-BY-SA/CC0 Wikimedia Commons licences the seed images use, and that dropping them leaves seed data without the attribution those licences require if the images are ever displayed; the stakeholder accepted this as a known, named tradeoff rather than an oversight. See DR-0018 for the full rationale.
