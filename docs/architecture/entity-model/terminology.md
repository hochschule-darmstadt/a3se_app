...
| `description` | plain-text string, 1–1200 Unicode characters | Short orientation text for a `TouristicProductItem`; normally one to four sentences, with flights typically using one to two sentences and accommodation or experience products allowed up to four. It is informational product content, not a booking condition or a replacement for structured properties. | Touristic Product Management; `TouristicProductItem` | optional for backward compatibility; when present, non-empty, plain text, and no more than 1200 characters; synthetic seed data supplies one description for every product | Project extension; issue #60 |
...
| `stock/airline/flight` | namespaced code | Sellable traveller capacity for one flight on one service date. | Inventory | common stock properties in TERM-005 | Project identifier; DR-0027 |
| `stock/accommodation/room-type` | namespaced code | Sellable traveller capacity for one accommodation room type on one service date. | Inventory | common stock properties in TERM-005 | Project identifier; DR-0027 |
...
| `order/header` | namespaced code | Root of one Travel Order. | Order Management | optional `orderNumber`; required `orderStatusCode` | Project identifier mapped to glossary Travel Order |
...
| `locality` | string, 1–100 Unicode characters | Human-readable, free-text place of a person or organisation, from a partial (e.g. city or country) to a full address. Seed data uses `City, Country` in English (e.g. `Lima, Peru`) so that text search finds both city and country. It is not an airport code and not a structured postal address. | Person or Partner Management; `Person`, `Organisation` | optional; do not infer an airport; no separate country property until structured addresses are specified (then ISO 3166-1 alpha-2, see TERM-006) | Project logical key; loosely corresponds to OTA Address concepts |
...
| `remainingCapacity` | non-negative integer | Traveller capacity not yet allocated to order positions; the persisted source of truth for availability. Each traveller on a position with allocated stock uses one unit, whether assigned before or after allocation; releasing the stock or deleting the position adds the units back. | Inventory; all current StockItem types | defaults to 1; must not exceed `capacityQuantity` | Project extension for INV-001; DR-0027 |
...
| `orderStatusCode` | namespaced code | Current Travel Order lifecycle status; replaces generic `status`. | Order Management; `order/header` | required; exactly one value from this row's five permitted codes; issue #21 introduces the CRUD API's `PUT` as the only current transition mechanism, so any code may currently follow any other -- ordering/guard rules remain future requirements work | Project value set; no external lifecycle adopted |
| `orderNumber` | string, 1–100 Unicode characters | Human-facing Travel Order reference; kept separate from the stable entity identifier. | Order Management; `order/header` | optional for draft creation; when present, preserve as a string | Project logical key; existing seeded order and staff acceptance data |
...
| `order/active` | namespaced code | At least one ordered travel service is underway; the order remains operationally active. | Order Management; `orderStatusCode` | permitted; distinguishes travel in progress from completed travel | Project extension grounded in UC-002, UC-013, and the glossary's Travel Order execution activities |
...
- DR-0020 closes DR-0017's deferred Inventory mismatch: every StockItem type is now `stock/` plus the complete suffix of the lowest-level represented product type. No legacy identifiers are accepted because the PoC is fully re-seeded rather than migrated. DR-0027 later retired the seat and room products: flight and room-type products own their dated stock directly.
...
| `addressLocalityName` → `locality`; `addressCountryName` removed | deprecated key mapping | One free-text place value replaces the city-only key and the undocumented, inconsistently seeded country key, so city and country searches both match. The PoC is re-seeded, not migrated. |
...
| `product/airline/flight/seat`, `product/accommodation/room-type/room`, `seatNumber`, `roomNumber` → removed | retired type and key mapping | Capacity is traveller-based per flight or room type and date; individual seats and rooms are not modelled (DR-0027). |
| `stock/airline/flight/seat` → `stock/airline/flight`; `stock/accommodation/room-type/room` → `stock/accommodation/room-type` | deprecated type mapping | Stock is owned by the flight or room-type product itself (DR-0027). |
| `heldQuantity`, `allocatedQuantity` → `remainingCapacity` | deprecated key mapping | One persisted capacity figure replaces competing counters; the `held` availability state is retired (DR-0027). |
...
Issue #56 documentation alignment (2026-09-16): a regression-test review found that this catalog still defined seat and room products, `seatNumber`, `roomNumber`, the seat and room stock types, and `heldQuantity`/`allocatedQuantity`, all of which the implementation removed in commit `9832ea0`. The project owner confirmed that the documentation should follow the implementation. The retired terms are listed in TERM-009, and DR-0027 records the decision. Earlier dated records in this section keep the identifiers that were valid at the time.
