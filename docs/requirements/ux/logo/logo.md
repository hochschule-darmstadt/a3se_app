...
| <img src="cct-wordmark-light.svg" width="160" alt=""> | [cct-wordmark.svg](cct-wordmark.svg) / [-light](cct-wordmark-light.svg) / [-dark](cct-wordmark-dark.svg) | Full wordmark: mark + "Christopher Columbus Travel". | `0 0 620 64` |
| <img src="cct-acronym-light.svg" width="80" alt=""> | [cct-acronym.svg](cct-acronym.svg) / [-light](cct-acronym-light.svg) / [-dark](cct-acronym-dark.svg) | Compact acronym lockup: mark + "CCT" — requested as an additional variant between the icon-only mark and the full wordmark. | `0 0 190 64` |
...
- **Application integration was out of scope for #24** (see its Scope boundaries). It was wired directly into the Customer/Staff shells during #27 phase 2 on stakeholder direction, rather than through a separate follow-up issue as originally suggested here: the wordmark (both the customer and, per stakeholder direction, the staff header, kept consistent rather than switching to the acronym lockup there) renders on the `navigation-strong` shell header/sidebar background. This still carries the unresolved risks below (no measured contrast/accessibility check, no visual-regression evidence); it is not yet independently validated integration.
- **Wordmark/acronym text enlarged relative to the ring** (2026-08-19, stakeholder feedback during #27 phase 2 integration): the wordmark's `font-size` grew from 24 to 34 and its `viewBox` from `0 0 480 64` to `0 0 620 64`; the acronym's `font-size` grew from 28 to 38 and its `viewBox` from `0 0 160 64` to `0 0 190 64`. The ring/dot mark geometry is unchanged. This was a visual judgment call, not a measured typographic fit -- worth a final look once rendered.
