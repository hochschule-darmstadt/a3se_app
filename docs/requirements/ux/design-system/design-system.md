...
|---|---|---|---|
| DS-CMP-001 | **Page/app shell and navigation:** skip link; header (logo, page identity, user-icon menu) with nav/main landmarks. Staff additionally provides a persistent sidebar landmark for managed-area navigation and no footer; customer additionally provides a footer landmark (imprint/legal), breadcrumb trail on deeper views, and reserves the advisor context (`aside`), with no sidebar. Mobile retains all customer destinations. | `AppShell`, `NavLink`, `Burger`, landmarks | WF-001/005/011/012; all views |
...
| DS-CMP-012 | **Incoming-reference navigation:** a positive distinct-record count is a labelled link to the accepted target view's exact identifier filter; zero is plain text. The link carries the complete originating relative URL in encoded `returnTo` state. The target renders a deliberate “Back to originating record” control that restores the prior filter, page, and selection. Counts are read-only snapshots and have no preview. | Shared Staff detail panels and `IncomingReferenceLinks`; API incoming-reference projection plus exact target filters | FR-017; issue #49 Phase 1 decision, 2026-09-01; DS-CMP-007 |
| DS-CMP-013 | **Responsive presentation image:** reserves a declared aspect ratio before load; selects a local, pre-generated rendition; uses `object-fit: cover` and a reviewed focal crop; shows a deterministic token-based fallback if unavailable. Above-the-fold informative imagery is eagerly requested with high priority; below-the-fold decorative imagery is lazy-loaded and has empty alternative text. Text overlays use a contrast scrim independent of the artwork. | Shared `ResponsiveImage` composition | VIEW-C-001; issue #25; NFR-001 |
...
| 2026-08-19 | Refined DS-CMP-001's contract wording (ID unchanged) to name the customer breadcrumb trail and customer footer landmark explicitly, reflecting the phase 1 shell wireframe refinement. | Issue #27 phase 1 (WF-012); reviewed and accepted by stakeholder direction, 2026-08-19 |
| 2026-08-19 | Implemented DS-CMP-001 as `CustomerShell`/`StaffShell` (`frontend/packages/ui/src/shell.tsx`), composed from shared logo/user-menu primitives; every Customer and Staff route now plugs into its profile shell instead of owning header/footer/nav markup. | Issue #27 phase 2; frontend unit tests (`app/lib/shell.test.tsx` in both apps), typecheck, and build all pass; pending the issue's phase 2 human review checkpoint |
...
| 2026-08-31 | Added visible text-labelled Back and Forward controls to the staff portal shell. They invoke the browser's existing history stack so users who do not think to use browser chrome have an equivalent portal affordance. | Issue #51 stakeholder clarification; shell history-control test |
| 2026-09-01 | Added DS-CMP-012 for accepted issue #49 incoming-reference counts, exact filtered cross-view links, encoded `returnTo`, and the target-view return control. Product → Travel Orders and supplier → Inventory were explicitly excluded by the Phase 1 decision. | Issue #49 stakeholder decision; implementation and generated API contract pending validation |
| 2026-09-01 | Extended DS-CMP-011 across the staff portal: area icons now accompany page titles, list/tree row labels, entity detail headlines, hierarchy/component chips, and create/contextual actions. Staff breadcrumbs remain intentionally absent; the sidebar communicates staff area context. Icons remain decorative and every surface retains a visible text label. | Issue #54 implementation; follow-up clarification |
| 2026-09-10 | Refined DS-CMP-007 shell history controls after portal review: the logo/page identity remains first, followed immediately by Back and Forward in browser order; both use the blue `action-secondary` token as navigation utilities. Customer `My travel` remains a warm primary action; the user control remains neutral against `navigation-strong`. | Stakeholder review; frontend shell implementation and typecheck/build evidence |
| 2026-09-16 | Added DS-CMP-013 and its shared `ResponsiveImage` implementation for VIEW-C-001. It keeps image loading, responsive source selection, reserved space, fallback treatment, and decorative/informative alternative-text policy consistent; the home route supplies only its approved local assets and crops. | Issue #25; image provenance and visual-review evidence in [portal imagery](../imagery/imagery.md) |
