...
| VIEW-S-003 | Tree-view product catalogue (breadcrumb-titled rows, lazy per-row expand to any depth, DB-derived type filter grouped by family, unified search) with lifecycle filter, selection, product detail with recursive component tree, draft edit, activate/retire (lifecycle status, WF-Q-014), type-selected create with optional parent component, validation, loading, empty, error, keyboard note — implementation is ahead of the low-fidelity frame here (see the tree-view follow-up note above) |
