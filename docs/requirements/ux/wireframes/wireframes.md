...
| VIEW-S-004 | Organisation/OrgaRole list with role-type/relationship-status filters, selection, Organisation detail with per-role sub-panels, separate Organisation/role edit, role deactivation (pending WF-Q-011), combined organisation+initial-role creation, validation, loading, empty, error, paging, keyboard note |
...
| WF-Q-011 | VIEW-S-004's "deactivate a role" action and relationship-status filter assume a `roleStatusCode`-equivalent property on `OrgaRole`; no such property exists yet (`backend/src/cct/resource_management/partner_management/models.py`'s `EmptySupplierRoleProperties`/`AirlineRoleProperties` carry none). Issue #30 phase 2 must add one following the WF-Q-010/`roleStatusCode` precedent before this frame's deactivate action and filter can be built as specified. | Issue #30; Architecture/Requirements |
