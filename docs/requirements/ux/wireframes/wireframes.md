...
|---|---|---|
| INV-001 | Inventory shall list pre-procured capacity with its linked TouristicProductItem, supplier context, service date, purchased capacity, remaining capacity, and derived availability state. | Accepted for issue #32; refines UC-006, UC-009, UC-017. |
...
| INV-003 | A Purchaser shall record a stock entry only for a flight, an accommodation room type, or another TouristicProductItem without components, with service date, sale price, currency, and capacity, and may later adjust those stock-owned values. | Accepted MVP boundary for UC-006; records already agreed capacity, not supplier negotiation. Flight and room-type products own traveller capacity directly (DR-0027). |
...
## MVP scope note (issue #36: client-side Travel and order placement)
...
| WF-012 | Both profiles share one page-shell chrome (skip link; header with logo placeholder, page identity, and user-icon menu; `main` landmark). Staff additionally provides a persistent left sidebar (`nav`, no footer); customer additionally provides a footer (imprint/legal placeholders, no sidebar), a breadcrumb trail (`nav aria-label="Breadcrumb"`) on deeper views, and the advisor rail per WF-011. Staff pages do not show shell breadcrumbs because the sidebar already communicates the current area and staff pages have no page hierarchy. | Issue #27 phase 1; DS-CMP-001; proposed presentation |
...
| VIEW-S-005 | Tree-view order list (order/header root rows, lazy-expand order/position child rows, mirroring VIEW-S-003's tree) with adjacent add/detail pane, editable header status, and chip-based position hierarchies; preparation, payment, document generation, cancellation policy, and active-travel actions remain deferred until their domain state and rules are accepted |
...
## VIEW-S-005 order refinement (issue #33)

- **WF-013 (accepted):** The order workspace uses the staff split-pane pattern: filters and results on the left, with information, add, or detail content on the right.
- **WF-014 (accepted):** Search, status, customer, product, service-date, and unresolved-allocation filters are evaluated by the database and represented in URL state; the browser does not filter only the fetched page.
- **WF-015 (accepted):** Each order position displays three direction-preserving chip hierarchies: position → stock → leaf product → product ancestors → supplier role/organisation; position → header → customer role/person; and position → traveller role/person.
- **WF-016 (accepted):** Customer relationships accept only `person/customer` roles, traveller relationships accept only `person/traveller` roles, and allocated stock represents only a leaf product. Seed validation enforces the same rules before mutation.
- **WF-017 (accepted):** Order creation records a header and customer role; header status is changed through the detail edit form. Position creation/allocation and traveller assignment remain API operations until their staff form interaction and selection rules are accepted.

Preparation, payment, document generation, cancellation behavior, and active-travel coordination named in the broader VIEW-S-005 proposal are not silently modeled as order properties. They remain deferred pending accepted domain states and workflow rules.

**Follow-up refinement (stakeholder review of the above, issue #33):** The list is now a tree view mirroring VIEW-S-003's product catalogue: each order/header is a root row, lazily expanding to its order/position children (label, service date, and unresolved state resolved the same way the detail panel's position chips already do), rather than a flat header-only list with positions visible only after opening the detail pane. WF-014's filter set changes accordingly: the free-text customer and product filters are removed (already covered by the combined search box), replaced by a product-type filter reusing the same alphabetical, DB-derived type list VIEW-S-003/S-007 use; search, status, service-date, and unresolved-allocation filters are unchanged. The "Unresolved only" checkbox now carries a visible description ("Has at least one position without allocated inventory") rather than relying on the label alone to convey what it filters. Order/position child rows are now themselves selectable (not just order/header rows), opening a standalone position detail in the same right pane -- reusing the order detail's own `.../detail` fetch and the shared `PositionHierarchies` chip rendering -- mirroring how selecting a product-tree child opens that child's own detail rather than staying on the parent's. A position with several travellers now shows one "Traveller hierarchy" row per traveller, rather than every traveller's chips crowded into a single row.

The order-header detail summary follows the same compact subitem pattern: positions are represented by one chip per position, with one position row per chip. An allocated position chip is labelled with its display-name chain and service date (`<product displayNameChain> · YYYY-MM-DD`); an unresolved position falls back to its position ID. Selecting a position chip opens that position's standalone detail, where the full position hierarchies remain available.
...
- **Proposals pending stakeholder review**:
  ...
  - Customer breadcrumbs are shown on deeper views and omitted on the customer entry point (VIEW-C-001, C-009), per the issue's "decide per view" guidance. Staff shell breadcrumbs were removed after review: the persistent sidebar communicates the current area and the staff views do not establish a page hierarchy. The product catalogue's record-context breadcrumb text remains separate domain content.
...
- **DS-CMP-001**: its contract already covered "header/nav/main/aside landmarks" and "page identity and current location" in principle; this refinement adds the footer landmark and breadcrumb trail for the customer profile, while staff location is expressed by the persistent sidebar. The design system's contract description was updated (ID unchanged) accordingly.
...
| WF-Q-009 | *Resolved 2026-08-31.* Staff shell breadcrumbs are not needed: the persistent sidebar conveys the current area and staff views do not define a page hierarchy. Record-context breadcrumb text in the product catalogue is not shell navigation. | Stakeholder direction; Requirements/UX |
...
AI refined the frames again on 2026-08-19 for issue #27 phase 1, introducing a shared shell chrome, profile-specific sidebar/footer, and customer breadcrumbs (WF-012) without changing any accepted view boundary, transition, or business behaviour. Staff shell breadcrumbs were subsequently removed on 2026-08-31 after review of the sidebar and flat staff navigation; the product-catalogue record-context breadcrumb remains. This refinement is likewise unreviewed by stakeholders and is the input to the issue's phase 1 human review checkpoint, not an accepted shell.
