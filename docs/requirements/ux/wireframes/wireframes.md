...
VIEW-S-007 follows the accepted staff managed-resource pattern: URL-backed filters and selection in a left list pane, with detail/edit or create mode in an independently scrolling right pane. The detail view distinguishes the linked product (what is supplied) from StockItem properties (when, quantity, state, and price). It renders the graph hierarchy as linked records in the same chip-style presentation as Product detail: StockItem → lowest-level product → product ancestors → supplier role → organisation. “Request supplier capacity” remains visible but disabled because negotiating or committing net-new capacity and supplier agreements require later business decisions. Availability state is derived as withdrawn/expired first, then shortfall for zero or negative availability, held when capacity is held, allocated when exhausted by allocation, and otherwise available. Withdrawal is the editable `inventoryStatusCode` value, not a separate detail action. A StockItem display name is `YYYY-MM-DD · <product displayNameChain>` in lists and detail headings.
