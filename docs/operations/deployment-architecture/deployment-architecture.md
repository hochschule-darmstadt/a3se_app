...
Logical modules remain packages and in-process dependencies inside api; they are not containers. One private project network connects the units. Only web and api publish loopback-bound ports. Docker service names are internal discovery names. `seed` is not a fourth long-running runtime unit -- it is the shared `cct-backend:local` image, run to completion once per invocation, under Compose's `seed` profile (excluded from the default `docker compose up`). API startup now uses the persisted `advisor-state` manifest to conditionally reset/reseed the disposable graph and rebuild the advisor index when seed or index fingerprints change; unchanged state is reused. Each explicit seed invocation still clears the disposable graph before loading the complete catalog and rebuilding the index; it never migrates or merges retained records. The `seed-reset` service is a compatibility alias for the same always-fresh behavior. A clean database and a current backend image are separate preconditions; rebuild the shared image after backend source changes. This conditional refresh is local-development behavior and is not a production migration or distributed coordination mechanism. `web` is not yet built -- issue #12 added `backend/Dockerfile` and the root `docker-compose.yml` for `neo4j`/`api`/`seed`/`seed-reset` only, leaving `web`'s containerization to a later frontend issue.
...
| startup, health, persistence, restart, reset, and recovery | manually smoke-tested for `neo4j`/`api`/`seed` (issue #12): `docker compose up` reaches healthy `neo4j` then `api`; every explicit seed command clears and reloads the full seed dataset; `seed-reset` remains a compatibility alias. Restart/recovery drills beyond that (DEP-005/DEP-006's fuller runbook) remain open |
