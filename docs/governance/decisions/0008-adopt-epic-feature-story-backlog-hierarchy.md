...
Stakeholder direction proposes `epic -> feature -> story`: an epic roughly corresponds to a DDD bounded context, a feature roughly corresponds to a use case within that context, and a story is a use-case slice such as the happy path or one exception path. The word "roughly" needs a precise governance rule so planning items cannot silently define requirements boundaries.
...
- an `epic` is associated with exactly one candidate or accepted bounded context;
- a `feature` has exactly one epic parent and represents exactly one use case in that context;
...
- Candidate bounded contexts may change during discovery, requiring backlog reconciliation.
